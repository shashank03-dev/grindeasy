...
// data (CC0) where the set carries the tool; the rest get a crafted code tile —
// three letters in the caps face inside a hairline ring — drawn at raster time
// in stage.tsx, so every tool still gets the same metal treatment.
//
// Tints are the per-tool metal color: mostly desaturated steel with one hint of
// the tool's own hue, so the section reads as one material that shifts as you
// scroll, not twelve stickers.
