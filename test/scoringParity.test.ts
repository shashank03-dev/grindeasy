...
import { COMBO_XP_HOURS, tierForXp, xpFromTotals } from "../src/tiers.js";

// The other half of this pair lives at web/lib/core/tiers.test.ts and reads the
// same fixture. The agent decides the tier shown on your Discord card; the
// server decides the tier shown on the leaderboard. They must never disagree.
const golden = JSON.parse(
  readFileSync(join(import.meta.dirname, "fixtures/scoring-vectors.json"), "utf8"),
) as {
  comboXpHours: number;
  vectors: Array<{
    activeMs: number;
...
describe("scoring parity with the leaderboard server", () => {
